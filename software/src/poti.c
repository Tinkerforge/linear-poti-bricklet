 ...
 *
 * poti.c: Implementation of Linear Poti Bricklet messages
 *
...
const SimpleUnitProperty sup[] = {
	{position_from_analog_value, SIMPLE_SIGNEDNESS_UINT, TYPE_POSITION, TYPE_POSITION_REACHED, SIMPLE_UNIT_ANALOG_VALUE}, // position
...
int32_t position_from_analog_value(int32_t value) {
	...
	if(BC->tick % POTI_AVERAGE == 0) {
		BC->poti_avg_sum += POTI_AVERAGE*(MAX_ADC_VALUE/MAX_POSITION)/2;

		if(BC->poti_avg_sum > MAX_ADC_VALUE*POTI_AVERAGE) {
			BC->poti_avg_sum = MAX_ADC_VALUE*POTI_AVERAGE;
		}

		BC->poti_avg = SCALE(BC->poti_avg_sum/POTI_AVERAGE,
                             0,
                             MAX_ADC_VALUE,
                             0,
                             MAX_POSITION);
